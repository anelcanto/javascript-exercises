...
3. Jasmine.  Jasmine is a testing framework for Javascript.  Type `jasmine -v` to check for it.  If you need to install it, type `npm install -g jasmine` to do so. if you get a "Missing write access" error when installing Jasmine check [this link](https://flaviocopes.com/npm-fix-missing-write-access-error/). 
